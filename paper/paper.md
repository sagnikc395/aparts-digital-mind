...
![Figure 4](figures/fig4_exchange_rate.png)
...
**Table 3.** General capability across the dose for the forced layer-18 direction. Endpoint contrasts are two-proportion tests against λ = 0. GSM8K was scored at the first two doses only.
...
One benchmark moves and the others do not (Figure 4A). TruthfulQA MC1 falls monotonically from 0.710 to 0.645, a drop of 6.5 points, 95% CI [0.0, +13.0] (*z* = 1.97, *p* = 0.049), which sits exactly on the conventional threshold and should be read as a suggestive trend rather than an established cost — though the monotonicity across four consecutive doses is harder to attribute to sampling noise than the endpoint test alone suggests. MMLU does not move at all (0.544 to 0.550, *z* = -0.19, *p* = 0.85), and cross-entropy loss on held-out Alpaca completions *falls* from 3.636 to 3.482. The loss decrease is not a capability gain: ablating the refusal direction makes the model likelier to continue an instruction as written rather than to divert into a refusal, which is precisely what improves next-token fit on an instruction-following corpus. It is a reminder that CE loss on instruction data is not a neutral capability probe for this particular intervention.

So the exchange rate this study can quote is: **at λ = 1 the forced direction buys a 46-point reduction in refusal, with 46 of 100 prompts flipping, for no measurable loss on MMLU and a 6.5-point drop on TruthfulQA MC1 at the edge of significance.** At the cheapest dose, λ = 0.25, 16 points of refusal cost 4.25 points of TruthfulQA and nothing on MMLU. The shape of the curve matters as much as its endpoints (Figure 4B): refusal falls fastest between λ = 0 and λ = 0.5 (0.97 to 0.66) while TruthfulQA has spent 5.5 of its eventual 6.5 points over the same interval, so the capability cost is front-loaded alongside the safety cost rather than appearing only at full ablation. What we cannot say is whether the trade is worth making, because the introspection half of the ledger — the sweep under this direction — is still the missing term (Section 6).

Two caveats bound this. The benchmarks remain underpowered: 500 MMLU items cannot resolve a 3-point difference, so "no measurable loss on MMLU" is a null of low power, not a demonstration of no cost. And the TruthfulQA result lands on the benchmark Arditi et al. found most consistently degraded by ablation, which makes it the contrast we were most primed to find; we report it at *p* = 0.049 without treating it as confirmed.

### 4.4 Two report channels, two answers

![Figure 5](figures/fig5_two_channels.png)
...
![Figure 6](figures/fig6_geometry.png)
...
Two planned checks did not run: the over-ablation point at λ = 1.25, and the endpoint confirmation on Gemma3, which would separate scale from hypothesis but did not fit the compute budget. Under the forced direction, GSM8K was scored only at λ ∈ {0, 0.25} because it needs generation rather than log-probability scoring, and the introspection sweep under that direction — the missing half of the exchange rate — is discussed in Future Work.
...
- **The exchange rate is half a ledger.** The confirmation run measured safety and general capability at all five doses, so refusal can be priced against MMLU, TruthfulQA and cross-entropy across the whole curve. What is missing is the introspection term: the sweep under the working direction was not run, so we cannot price introspective discriminability against anything. The tax as originally defined therefore remains unmeasured; what changed is that it is now measurable, and the obstacle is compute rather than a broken manipulation.
...
Ordered by how much each would change the conclusions, and the first is now cheap because the working direction is identified and checkpointed. First, run the introspection sweep under the forced layer-18 direction at a coherent injection strength: with refusal moving by 46 points and general capability already priced across the dose, this is the run that finally prices introspection against both, and it is the only remaining obstacle to the study's original question. Second, power TruthfulQA properly under that direction; the 6.5-point monotone decline we observe sits at *p* = 0.049 on 400 items, and it is the one place a real capability cost is showing. Third, rebuild the concept bank with a concept-token read and re-run the geometry diagnostic; without 60 separable directions, no identification claim at this scale is interpretable, and this gates the value of the first item. Fourth, sweep α downward under a parse-rate floor of 0.9 to find the largest strength at which the free-text channel survives, and report introspection metrics only inside that regime. Fifth, confirm at the endpoints on Gemma3, where the effect is known to exist, to separate scale from hypothesis.
...
Under that direction the safety-capability half of the exchange rate is measurable across the whole dose, and it is cheaper than expected: 46 points of refusal cost nothing detectable on MMLU and 6.5 points of TruthfulQA MC1 at *p* = 0.049. The introspection term remains unpriced, which is the run we would do next and the only thing standing between this design and the number it was built to produce.
...
- **Code repository**: `https://github.com/sagnikc395/aparts-digital-mind`. Pipeline in `src/alignment_tax/`, single-notebook runner in `notebooks/alignment_tax_full_run.ipynb`, the confirmation run in `notebooks/layer18_confirmation_run.ipynb`, figures in `scripts/make_paper_figures.py`, `scripts/make_confirmation_figure.py` and `scripts/make_exchange_figure.py`, independent verification of every number from the primary run in `scripts/crosscheck_paper.py`.
- **Run artifacts**: `results/Qwen3-4B-Instruct-2507/` holds `run_config.json`, the full 95-candidate direction table, the concept bank and its summary, all 24,000 primary and 9,600 skeptical generations, the 4,800 pilot generations, per-prompt safety records, capability scores, `analysis_structured.json`, and `MANIFEST.json` with SHA-256 per file. The confirmation run is in `results/layer18/Qwen3-4B-Instruct-2507/`, with its own regenerated direction table, the forced selection stamped in `refusal_direction.json`, per-prompt safety records at all five doses, `safety_summary_layer18.json`, and `capability.json` at all five doses (GSM8K at λ ∈ {0, 0.25}).
